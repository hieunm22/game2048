...
export function moveLeft(currentMatrix) {
	...
	for (let index = 0; index < currentMatrix.length; index++) {
		...
		const nearestMovableIndexForLeftMove = findNearestMovableIndexForLeftMove(
			lastIndexNotEmpty,
			index,
			currentMatrix
		)
...
export function moveRight(currentMatrix) {
	...
	for (let index = currentMatrix.length - 1; index >= 0; index--) {
		...
		const element = currentMatrix[index]
		if (element === 0) continue
		const nearestMovableIndexForRightMove = findNearestMovableIndexForRightMove(
			lastIndexNotEmpty,
			index,
			currentMatrix
		)
...
export function moveDown(currentMatrix) {
	...
	for (let j = 0; j < currentMatrix.length; j++) {
		...
		if (index >= matrixSize * (matrixSize - 1)) {
			lastIndexNotEmpty = index
			index =
				index < matrixSize ? index - matrixSize + indexAtBottomRightCorner : index - matrixSize
			continue
		}
		const element = currentMatrix[index]
		if (element === 0) {
		...
		const nearestMovableIndexForDownMove = findNearestMovableIndexForDownMove(
			lastIndexNotEmpty,
			index,
			currentMatrix
		)
...
export function moveUp(currentMatrix) {
	...
	for (let j = 0; j < currentMatrix.length; j++) {
		...
		if (index <= matrixSize - 1) {
			lastIndexNotEmpty = index
			index =
				index !== indexAtBottomRightCorner - matrixSize
					? (index + matrixSize) % indexAtBottomRightCorner
					: index + matrixSize
			continue
		}
		const element = currentMatrix[index]
		...
		const nearestMovableIndexForUpMove = findNearestMovableIndexForUpMove(
			lastIndexNotEmpty,
			index,
			currentMatrix
		)
